...
fn solve() -> usize {
    ...
    for k1 in a..=z {
        for k2 in a..=z {
            for k3 in a..=z {
                let decrypt: Vec<u8> = CIPHER
                    .iter()
                    .zip((&[k1, k2, k3]).iter().cycle())
                    .map(|(c, k)| c ^ k)
                    .collect();
                if decrypt.windows(5).any(|window| window == search.as_bytes()) {
                    //dbg!(k1, k2, k3);
                    //dbg!(String::from_utf8(decrypt.clone()));
                    return decrypt.iter().map(|d| (*d) as usize).sum::<usize>();
                }
            }
        }
    }
