...
static HANDS: &[(&'static str, &'static str)] = &[
...
fn test() {
    ...
    for hand in hands {
        ...
        let hand0 = PokerHand::from(hand0);
        let hand1 = PokerHand::from(hand1);
...
//suits are _never_ ranked in poker
...
impl PokerCardValue {
    fn next(&self) -> Option<Self> {
        Some(match *self {
            PokerCardValue::N(10) => PokerCardValue::Jack,
            PokerCardValue::N(n) => PokerCardValue::N(n + 1),
            PokerCardValue::Jack => PokerCardValue::Queen,
            PokerCardValue::Queen => PokerCardValue::King,
            PokerCardValue::King => PokerCardValue::Ace,
            PokerCardValue::Ace => return None,
        })
    }
}
...
enum PokerHandRank {
    HighCard(PokerCardValue),
    OnePair(PokerCardValue),
    //first higher pair, then lower pair
    TwoPairs(PokerCardValue, PokerCardValue),
    ThreeOfAKind(PokerCardValue),
    //the lowest value of the straight
    Straight(PokerCardValue),
    Flush,
    //first value of three, then value of two
    FullHouse(PokerCardValue, PokerCardValue),
    FourOfAKind(PokerCardValue),
    StraightFlush(PokerCardValue),
    RoyalFlush,
}
#[derive(Copy, Clone, Debug)]
struct PokerHand {
...
impl PokerHand {
    fn from(hand: &str) -> Self {
        let mut cards = [PokerCard {
            suit: PokerCardSuit::D,
            value: PokerCardValue::N(2),
        }; 5];
        for (i, card) in hand.split_ascii_whitespace().enumerate() {
            let c0 = card.chars().nth(0).unwrap();
            let c1 = card.chars().nth(1).unwrap();
            match c0 {
                '2' => cards[i].value = PokerCardValue::N(2),
                '3' => cards[i].value = PokerCardValue::N(3),
                '4' => cards[i].value = PokerCardValue::N(4),
                '5' => cards[i].value = PokerCardValue::N(5),
                '6' => cards[i].value = PokerCardValue::N(6),
                '7' => cards[i].value = PokerCardValue::N(7),
                '8' => cards[i].value = PokerCardValue::N(8),
                '9' => cards[i].value = PokerCardValue::N(9),
                'T' => cards[i].value = PokerCardValue::N(10),
                'J' => cards[i].value = PokerCardValue::Jack,
            ...
            match c1 {
                'D' => cards[i].suit = PokerCardSuit::D,
...
impl PokerHandRank {
    fn from(hand: PokerHand) -> Self {
        let cards = hand.cards;
        let straight = cards[0].value.next() == Some(cards[1].value)
            && cards[1].value.next() == Some(cards[2].value)
            && cards[2].value.next() == Some(cards[3].value)
            && cards[3].value.next() == Some(cards[4].value);
        ...
        let pairs_b: [bool; 4] = [
            ...
            cards[1].value == cards[2].value && cards[0].value != cards[1].value,
            cards[2].value == cards[3].value && cards[1].value != cards[2].value,
            cards[3].value == cards[4].value && cards[2].value != cards[3].value,
        ];

        let pairs = pairs_b.iter().filter(|b| **b).count();
        let three = cards[0].value == cards[2].value
            || cards[1].value == cards[3].value
            || cards[2].value == cards[4].value;

        let four = cards[0].value == cards[3].value || cards[1].value == cards[4].value;

        if cards[0].value == PokerCardValue::N(10) && straight && flush {
            PokerHandRank::RoyalFlush
        } else if straight && flush {
            PokerHandRank::StraightFlush(cards[0].value)
        } else if four {
            PokerHandRank::FourOfAKind(cards[2].value)
        } else if three && pairs == 2 {
            if cards[0].value == cards[2].value {
                PokerHandRank::FullHouse(cards[2].value, cards[3].value)
            } else {
                PokerHandRank::FullHouse(cards[2].value, cards[0].value)
            }
        } else if flush {
            PokerHandRank::Flush
        } else if straight {
            PokerHandRank::Straight(cards[0].value)
        } else if three {
            PokerHandRank::ThreeOfAKind(cards[2].value)
            ...
            if pairs == 2 {
                let p2 = pairs_b
                    ...
                    .unwrap()
                    .0;
                PokerHandRank::TwoPairs(cards[p2].value, cards[p1].value)
            } else {
                PokerHandRank::OnePair(cards[p1].value)
            }
        } else {
            PokerHandRank::HighCard(cards[4].value)
        }
    }
}
...
impl PartialOrd for PokerHand {
    fn partial_cmp(&self, other: &Self) -> Option<core::cmp::Ordering> {
        let s = PokerHandRank::from(*self);
        let o = PokerHandRank::from(*other);
        //dbg!(&s, &o);
        match (&s, &o) {
            (PokerHandRank::Flush, PokerHandRank::Flush) => {
                dbg!(&self, &other, &s, &o);
                panic!("Flush Ranking not implemented");
            }
            _ => {}
        }
        match s.cmp(&o) {
            ...
            core::cmp::Ordering::Equal => {
                ...
                Some(
                    self.cards
                        ...
                        .rev()
                        .map(|c| c.value)
                        .cmp(other.cards.iter().rev().map(|c| c.value)),
                )
            }
        }
    }
